...
func TestRhdmAuthoringHAEnvironment(t *testing.T) {
	...
	assert.Equal(t, constants.ImageRegistry+"/"+constants.IBMBamoeImageContext+"/"+constants.IBMBamoeImagePrefix+"-businesscentral-rhel8"+":"+cr.Status.Applied.Version, env.Console.Deployments[0].Spec.Template.Spec.Containers[0].Image)
	for i := 0; i < len(env.Servers); i++ {
...
func checkAuthoringHAEnv(t *testing.T, cr *api.KieApp, env api.Environment, productPrefix string) {
	...
	assert.Equal(t, appsv1.DeploymentStrategyTypeRecreate, env.Console.Deployments[0].Spec.Strategy.Type)
...
func TestRhdmProdImmutableEnvironment(t *testing.T) {
	...
	assert.Nil(t, env.Console.Deployments)
...
func TestRhpamProdWithSmartRouterWithSSLDisabled(t *testing.T) {
	...
	assert.Equal(t, env.SmartRouter.Deployments[0].Spec.Strategy.Type, appsv1.DeploymentStrategyTypeRollingUpdate)
	assert.Equal(t, env.SmartRouter.Deployments[0].Spec.Strategy.RollingUpdate.MaxSurge, &intstr.IntOrString{Type: 1, IntVal: 0, StrVal: "100%"})

	assert.Equal(t, "test-rhpamcentrmon", env.Console.Deployments[0].ObjectMeta.Name)
	assert.Equal(t, "test-smartrouter", env.SmartRouter.Deployments[0].ObjectMeta.Name)
	assert.Equal(t, bcmImage+":"+cr.Status.Applied.Version, env.Console.Deployments[0].Spec.Template.Spec.Containers[0].Image)
	assert.Equal(t, appsv1.DeploymentStrategyTypeRollingUpdate, env.Console.Deployments[0].Spec.Strategy.Type)
	assert.Equal(t, &intstr.IntOrString{Type: 1, IntVal: 0, StrVal: "100%"}, env.Console.Deployments[0].Spec.Strategy.RollingUpdate.MaxSurge)
...
func TestRhpamProdImmutableJMSEnvironment(t *testing.T) {
	...
	assert.Equal(t, "OpenShiftStartupStrategy", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_STARTUP_STRATEGY"), "Variable should exist")
	assert.Equal(t, "test-jms-kieserver", env.Servers[0].Deployments[0].Name)
	assert.Equal(t, "test-jms-kieserver-postgresql", env.Databases[0].Deployments[0].Name)
	assert.Equal(t, "test-jms-kieserver-amq", env.Servers[0].Deployments[1].Name)
	assert.Equal(t, "amq-jolokia-console", env.Servers[0].Routes[1].Name)
	assert.True(t, env.Servers[0].Routes[1].Spec.TLS == nil)
	testAMQEnvs(t, env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0].Env, env.Servers[0].Deployments[1].Spec.Template.Spec.Containers[0].Env)
...
func TestRhpamProdImmutableJMSEnvironmentExecutorDisabled(t *testing.T) {
	...
	assert.Equal(t, "amq-jolokia-console", env.Servers[0].Routes[1].Name)
	assert.True(t, env.Servers[0].Routes[1].Spec.TLS == nil)
	assert.Equal(t, "false", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_EXECUTOR_JMS"), "Variable should exist")
	assert.Equal(t, "true", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_EXECUTOR_JMS_TRANSACTED"), "Variable should exist")
	assert.Equal(t, "true", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_JMS_ENABLE_AUDIT"), "Variable should exist")
	assert.Equal(t, "queue/CUSTOM.KIE.SERVER.AUDIT", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_JMS_QUEUE_AUDIT"), "Variable should exist")
	assert.Equal(t, "true", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_JMS_ENABLE_SIGNAL"), "Variable should exist")
	assert.Equal(t, "queue/CUSTOM.KIE.SERVER.SIGNAL", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_JMS_QUEUE_SIGNAL"), "Variable should exist")
	assert.Equal(t, "queue/KIE.SERVER.REQUEST, queue/KIE.SERVER.RESPONSE, queue/CUSTOM.KIE.SERVER.SIGNAL, queue/CUSTOM.KIE.SERVER.AUDIT", getEnvVariable(env.Servers[0].Deployments[0].Spec.Template.Spec.Containers[0], "AMQ_QUEUES"), "Variable should exist")
...
func commonRhpamAuthoringAssertions(t *testing.T, env api.Environment, cr *api.KieApp) {
	...
	assert.Equal(t, string(appsv1.DeploymentStrategyTypeRolling), string(env.Servers[len(env.Servers)-1].Deployments[0].Spec.Strategy.Type), "The DC should use a Rolling strategy when using the H2 DB")
...
func TestConstructSmartRouterObject(t *testing.T) {
	...
	assert.Equal(t, fmt.Sprintf("%s-smartrouter-rhel8:%s", constants.IBMBamoeImagePrefix, cr.Status.Applied.Version), env.SmartRouter.Deployments[0].Spec.Triggers[0].ImageChangeParams.From.Name)
	for i := range sampleEnv {
...
func TestConstructServerObject(t *testing.T) {
	name := "test"
	{
		...
		assert.Equal(t, fmt.Sprintf("%s-businesscentral-rhel8:%s", constants.IBMBamoeImagePrefix, cr.Status.Applied.Version), env.Console.Deployments[0].Spec.Triggers[0].ImageChangeParams.From.Name)
		for i := range sampleEnv {
...
func TestRhdmTrialConsoleEnv(t *testing.T) {
	...
	assert.Equal(t, fmt.Sprintf("%s-businesscentral-rhel8:%s", constants.IBMBamoeImagePrefix, cr.Status.Applied.Version), env.Console.Deployments[0].Spec.Triggers[0].ImageChangeParams.From.Name)
	assert.Equal(t, fmt.Sprintf("%s-kieserver-rhel8:%s", constants.IBMBamoeImagePrefix, cr.Status.Applied.Version), env.Servers[0].Deployments[0].Spec.Triggers[0].ImageChangeParams.From.Name)
...
func checkObjectLabels(t *testing.T, cr *api.KieApp, object api.CustomObject, component string, subcomponent string) {
	for _, dc := range object.Deployments {
...
func checkImageNames(cr *api.KieApp, imageName, imageURL string, t *testing.T) {
	...
	assert.Nil(t, err, "Error getting prod environment")
	assert.Len(t, env.Servers, 2, "Expect two KIE Servers to be created based on provided build configs")
	_ = imageName // kept for signature compatibility; no longer used directly
	...
	if len(env.ProcessMigration.Deployments) > 0 {

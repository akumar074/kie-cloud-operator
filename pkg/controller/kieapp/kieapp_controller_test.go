...
func TestStatusDeploymentsProgression(t *testing.T) {
	...
	service.ListFunc = func(ctx context.Context, list client.ObjectList, opts ...client.ListOption) error {
		...
		if err == nil && reflect.TypeOf(list) == reflect.TypeOf(&v1.DeploymentList{}) {
			for index := range list.(*v1.DeploymentList).Items {
				dep := &list.(*v1.DeploymentList).Items[index]

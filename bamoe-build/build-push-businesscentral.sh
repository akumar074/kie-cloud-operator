...
# Build and push script for BAMOE Business Central with updated JAR files
# This script builds a custom Business Central image with updated controller, drools-core, and drools-serialization-protobuf JARs

set -e

# Configuration
IMAGE_NAME="quay.io/abkuma/bamoe-businesscentral-rhel9"
IMAGE_TAG="8.0.9"
FULL_IMAGE="${IMAGE_NAME}:${IMAGE_TAG}"

echo "=========================================="
echo "Building BAMOE Business Central Custom Image"
echo "=========================================="
...
if [ ${MISSING_JARS} -eq 1 ]; then
    echo ""
    echo "ERROR: One or more JAR files not found in Maven repository"
...
echo ""
echo "Base Images:"
echo "  - AMD64: registry.redhat.io/ibm-bamoe/bamoe-businesscentral-rhel9:8.0.9-3"
echo "  - ARM64: quay.io/r_anand/bamoe-businesscentral-rhel9:8.0.9"
echo ""

# Build and push multi-platform image using Dockerfile.businesscentral
echo "Building and pushing ${FULL_IMAGE}..."
docker buildx build \
    --platform linux/amd64,linux/arm64 \
    --file Dockerfile.businesscentral \
    --tag "${FULL_IMAGE}" \
    --push \
    .
